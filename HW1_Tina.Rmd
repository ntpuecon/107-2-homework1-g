...
>因為*可修個經*的條件其一為「經原及格」，表示經原的學習成果會與個經的學習有關係，表示*經原PR*與*個經PR*有關係，且應是正向關係，若只有放*可修個經*為虛擬變數，只可以區分大一是否有過「經原」或者大一後重修「經原」，在一般情況下不論是第一次修或者重修，只要在這門課拿到及格分數，即被（世人）認定為有這門課的學習成效及具備與之相關的能力，故無法分辯兩者能力上的區別，所以需要透過*經原PR*這個變數來表示區分每個人在「經原」這門課上的學習果效與能力。
...
```{r ans17}
library(dplyr)
summarise(group_by(hw1Data,可修個體),mean(經原PR,na.rm=T),mean(個經PR,na.rm=T))
```

## 1.8 偏誤方向

請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)_下降__(上升/下降) (2)__1__百分點，其值比未控制**經原PR**時還 (3)__低__(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)__upward bias__(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)__正向__(正向/反向)關連
  2. **可修個體**=1的族群，其**經原PR**平均較(6)__高__(高/低)
  
故若不控制**經原PR**，因**可修個體**=1與=0兩群人也會分別帶有 **經原PR**  (7)_高與低___(高與低/低與高)特質，所以**可修個體**=1的人其**個經PR**高，有部份是因其**經原PR**也較(8)_高__(高/低)所至。
...
假設$E(\epsilon_i|x_i)=0$，證明上題設定下：
 ...
 so that $=1/[\sum{({x_i-\bar{x}})^2})^2\sum{E[({x_i-\bar{x}})^2\epsilon_i^2|x]} = \sigma_i^2/\sum{({x_i-\bar{x}})^2} = \sigma_i^2/(n_0n_1/n_0+n_1) = n\sigma_i^2/n_0n_1$
  ...
  $V(\hat{\beta_1}|x)=V(\bar{y_1}-\bar{y_0})|x)$
                   $=V(\bar{y_1})+V(\bar{y_0})-2cov(\bar{y_1},\bar{y_0})$
